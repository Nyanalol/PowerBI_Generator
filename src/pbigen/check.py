...
def check_report(report_dir: Path, semantic_model_dir: Path | None) -> list[Finding]:
    ...
    for page_json in pages_dir.glob("*/page.json"):
        ...
        for vjson in (page_json.parent / "visuals").glob("*/visual.json"):
            ...
            if vtype == "cardVisual" and h < 100:
                # Comprobado en Desktop 2.157: con etiqueta arriba y cifra de 40 pt hacen falta unos
                # 100 px; por debajo, la tarjeta muestra un esqueleto de guiones en vez del valor.
                findings.append(Finding("warning", "CARD_TOO_SHORT", f"tarjeta de {h:.0f} px de alto; necesita >= 100 px para mostrar el valor", rel))
            if vtype == "slicer" and h < 76:
                findings.append(Finding("warning", "SLICER_TOO_SHORT", f"slicer de {h:.0f} px; con cabecera necesita >= 76 px", rel))
