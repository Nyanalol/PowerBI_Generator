...
| R21 | `The visual has unrecognized fields` en un ranking con Top N | En el subconsulta del filtro, la categoría y la columna de orden viven en tablas distintas y compartían alias | Un alias por tabla en `From` + test `test_topn_uses_one_alias_per_table` |
| R22 | El filtro no se aplica o el validador avisa de `PBIR_FILTER_ENTITY_IN_WHERE` | Dentro de `filter.Where` la fuente es el alias de `From` (`Source`), no la entidad (`Entity`) | `_categorical_filter` usa el alias; test en `test_v2_filters_topn_and_new_visual_types` |
| R20 | El informe parece una plantilla aunque los datos y colores sean correctos | Todos los visuales con el mismo marco, el mismo peso y la misma composición en todas las páginas | Reglas de retícula y jerarquía en `designer.md`: visual dominante, menos del 40 % con borde, composición variada, escalón tipográfico de 10 pt |
